# Análisis Inteligente del Mercado Inmobiliario en Madrid

**Proyecto presentado a la II Edición de los Premios a la Reutilización de Datos Abiertos del Ayuntamiento de Madrid (2026)**

* **Autor:** Rodrigo Alonzo Peña Vigil
* **Categoría:** Estudios, investigaciones e ideas

## 🎯 Objetivo del Proyecto
Este proyecto presenta una solución tecnológica para facilitar la comprensión de la evolución del mercado inmobiliario en Madrid. A través de un cuadro de mando interactivo basado en datos abiertos, la herramienta permite explorar series temporales, analizar rentabilidad y comparar distritos de forma intuitiva.

## 📊 Cuadro de Mando Interactivo
[▶️ Haz clic aquí para acceder y probar el cuadro de mando en Power BI](https://app.powerbi.com/view?r=eyJrIjoiZWVhOTM4ZmUtYWRiMy00N2ZmLTg3NTktZDkyZjJiODExYjI3IiwidCI6IjZhZmVhODVkLWMzMjMtNDI3MC1iNjlkLWE0ZmIzOTI3YzI1NCIsImMiOjl9&pageName=dc5e9c4eecefccf19af9)

## 📺 Vídeo Demostrativo
[▶️ Haz clic aquí para ver el vídeo explicativo del proyecto](Pega_aqui_el_enlace_de_tu_video)

## 📂 Documentación
En este repositorio se encuentra alojada la memoria técnica del proyecto en formato PDF, la cual detalla las fuentes de datos utilizadas, la metodología aplicada y las conclusiones principales del estudio.
